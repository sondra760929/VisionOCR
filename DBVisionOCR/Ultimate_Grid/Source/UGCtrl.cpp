		...
		{	
			...
			{	// reset the size object to reflect cell's width and height
				...
				if(size.cx > bestWidth[x-startCol])
					bestWidth[x-startCol] = size.cx + (m_GI->m_margin * 2);
			}
		}
...
void CUGCtrl::OnDrawFocusRect(CDC *dc,RECT *rect){
	...
	//dc->DrawEdge(rect, BDR_RAISEDINNER | BDR_SUNKENOUTER, BF_RECT);

	CRect current_rect(rect);
	InflateRect(current_rect, -1, -1);
	CPen pen(PS_SOLID, 2, RGB(0, 0, 255));
	CPen* pOldPen = dc->SelectObject(&pen);
	dc->MoveTo(current_rect.left, current_rect.top);
	dc->LineTo(current_rect.right, current_rect.top);
	dc->LineTo(current_rect.right, current_rect.bottom);
	dc->LineTo(current_rect.left, current_rect.bottom);
	dc->LineTo(current_rect.left, current_rect.top);
...
//#ifdef _UNICODE
//	USES_CONVERSION;
//	ifs.open(W2A(fileName), ios::binary);
//#else
